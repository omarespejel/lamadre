...
cd "$ROOT/rust"
cargo run --bin simulate_swap 2>&1 | tail -4
HASHLOCK=$(cargo run --bin simulate_swap 2>&1 | grep -o 'hashlock=\[[^]]*\]' | head -1 | sed 's/hashlock=//')
CK=$(cargo run --bin simulate_swap 2>&1 | grep -o 'c_k=\[[^]]*\]' | head -1 | sed 's/c_k=//')
echo ""
...
echo ""
echo "When custom Lamadre deployed:"
echo "aztec-wallet send create_lock --from lamadre-demo --contract-address <LAMADRE> --args $HASHLOCK $CK 1234567890"
